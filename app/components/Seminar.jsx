          ...
          <div className="w-[390px] h-full rounded-xl">
            <Slider {...settings} ref={(slider) => setSlider1(slider)}>
              {firstHalfEventData.map((item, index) => (
                ...
                >
                  <div className="relative h-56 mx-4 -mt-6 overflow-hidden text-white shadow-lg bg-clip-border rounded-xl bg-pink-gray-500 shadow-blue-gray-500/40">
                    <img
                      src={item.imageUrl}
                      alt={item.title}
                  ...
                  <div className="p-6 h-[200px] ">
                    <h5 className="block mb-2 font-sans text-xl antialiased font-semibold leading-snug tracking-normal text-pink-gray-900">
                      {item.title}
                    </h5>
            ...
            <Slider {...settings} ref={(slider) => setSlider2(slider)}>
              {secondHalfEventData.map((item, index) => (
                <div
                  ...
                  className="relative flex flex-col mt-6 text-gray-700 bg-white shadow-md bg-clip-border rounded-xl w-96"
                >
                  <div className="relative h-56 mx-4 lg:-mt-6 overflow-hidden text-white shadow-lg bg-clip-border rounded-xl bg-pink-gray-500 shadow-blue-gray-500/40">
                    <img
                      src={item.imageUrl}
                      alt={item.title}
                      className="w-full h-full object-cover"
    ...
    <li className="border-gray-400 flex flex-col">
      <Link href={`/events/${uuid}`}>
        <div className="w-full flex gap-2 px-2 ">
          <div className="p-4 bg-pink-400 text-center">
            <p>9 April</p>
          </div>
          <div className="w-full justify-center p-2 items-start flex flex-col">
            ...
            <div className="flex justify-start py-4 items-center text-sm font-semibold">
              <h1 className="px-2 p-1 font-semibold rounded-lg capitalize bg-pink-200">
                {type}
              </h1>
            </div>
          </div>
